...
def teacher_tab_manage_subjects():
    ...
    if subjects:
        for sub in subjects:
            ...
            # FIX: Indented this callback to be INSIDE the for loop
            def share_btn():
                if st.button(f"Share Code: {sub['name']}", key=f"share_{sub['subject_code']}", icon=":material/share:"):
                    share_subject_dialog(sub['name'], sub['subject_code']) # Ensure this is imported!
                    st.space()

            # FIX: Indented the component call to be INSIDE the for loop
            subject_card(
                name = sub['name'],
